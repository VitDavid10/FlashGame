/*
 * Daily quests rotativas. Cada día (UTC) el servidor elige 4 retos del pool de forma
...
// Elige 4 retos del pool para `date`, sin repetir.
function pickFor(date) {
    const idxs = [];
    let seed = hash(date);
    const pool = POOL.slice();
    while (idxs.length < 4 && pool.length) {
